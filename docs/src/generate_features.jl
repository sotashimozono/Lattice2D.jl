...
for i in 1:num_sites(l)
    pos = position(l, i)
    scatter!(p, [pos[1]], [pos[2]], color=:lightgray, markersize=10, markerstrokewidth=2, markercolor=:white)
end
...
plot!(p3, px, py, fill=true, color=:lightgreen, fillalpha=0.5, linewidth=2, linecolor=:green)
