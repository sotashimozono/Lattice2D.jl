...
struct TightBindingModel{T}
    ...
    function TightBindingModel(H::SparseMatrixCSC{T,Int}, positions::Vector{Vector{Float64}}) where T
        n = size(H, 1)
        @assert n == length(positions) "Hamiltonian size must match number of positions"
        is_herm = ishermitian(H)
        new{T}(H, positions, n, is_herm)
    end
end
...
"""
function build_fibonacci_tight_binding(qc_data::QuasicrystalData{1,T}; 
                                       t::Real=1.0, 
                                       onsite_modulation::Real=0.0) where T
...
- `TightBindingModel`: Tight-binding model with nearest-neighbor hopping
"""
function build_penrose_tight_binding(qc_data::QuasicrystalData{2,T}; 
                                     t::Real=1.0,
                                     cutoff::Real=1.5) where T
    n = length(qc_data.positions)
...
"""
function compute_dos(eigenvalues::Vector{<:Real}; 
                    n_bins::Int=100, 
                    sigma::Real=0.1)
